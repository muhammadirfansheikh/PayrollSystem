...
function GetEmployee() {
    ...
    var GroupByValue = '';
    if ($('.ddlGroupBy').val() == "clsDepartment") { GroupByValue = 1; }
    if ($('.ddlGroupBy').val() == "clsLocation") { GroupByValue = 2; }
    if ($('.ddlGroupBy').val() == "clsCostCenter") { GroupByValue = 3; }
    if ($('.ddlGroupBy').val() == "clsSapCostCenter") { GroupByValue = 4; }
    ...
    if (CompanyId > 0) {
        ...
        var IncreaseRate = parseFloat($('.txtIncreaseRate').val());

        if (IncreaseRate != null && IncreaseRate > 0) {
            if (PremiumRate != null && PremiumRate > 0) {
                if (PremiumRateGPA != null && PremiumRateGPA > 0) {
                    ProgressBarShow();
                    ClearReport();
                    var service = new HrmsSuiteHcmService.HcmService();
                    service.report_GetBudgetSummaryReportWithIncrease(CompanyId, PremiumRate, IncreaseRate,
                        PayrollMonth, EmployeeCode, PremiumRateGPA, GroupByValue, onGetBudgetSummaryReportWithIncrease, null, null);
                }
                else {
            ...
            showError('Please enter increase rate.');
        }
...
function onGetBudgetSummaryReportWithIncrease(result) {
    var GroupByValue = $('.ddlGroupBy').val();
    if (GroupByValue == "clsDepartment") {
        $('.Headercode').text('DepartmentCode');
        $('.HeaderName').text('Department');
    }
    if (GroupByValue == "clsLocation") {
        $('.Headercode').text('LocationCode');
        $('.HeaderName').text('Location');
    }
    if (GroupByValue == "clsCostCenter") {
        $('.Headercode').text('CostCenterCode');
        $('.HeaderName').text('CostCenter');
    }
    if (GroupByValue == "clsSapCostCenter") {
        $('.Headercode').text('SapCostCenterCode');
        $('.HeaderName').text('SapCostCenter');
    }
    ...
    //}
    var divTbodyGoalFund = $('.tbodybudgetsummarywithincreaseListing').html('');
    $('#budgetsummarywithincreaseListing').tmpl(res).appendTo(divTbodyGoalFund);
        ...
        $('.trList').each(function () {
            ...
            if ($(this).is(':first-child')) {
                ...
                if ($(this).is(':last-child')) {
                    ...
                    //$("<tr class='success'><th colspan=" + ColSpan + " style='text-align:left'>" + GroupByName + " : " + GroupByItem + "</th><th style='text-align:right'>" + _Sum1 + "</th><th style='text-align:right'>" + _Sum2 + "</th><th style='text-align:right'>" + _Sum3 + "</th><th style='text-align:right'>" + _Sum4 + "</th><th style='text-align:right'>" + _Sum5 + "</th><th style='text-align:right'>" + _Sum6 + "</th><th style='text-align:right'>" + _Sum7 + "</th><th style='text-align:right'>" + _Sum8 + "</th><th style='text-align:right'>" + _Sum9 + "</th><th style='text-align:right'>" + _Sum10 + "</th><th style='text-align:right'>" + _Sum1 + "</th><th style='text-align:right'>" + _Sum12 + "</th></tr>").insertAfter($(this));
                    //i = -1;
                    _Sum1 = 0;
                    _Sum2 = 0;
                    _Sum3 = 0;
    ...
    SetReportHeader('Budget Summary With Increase Report', 10, '');
...
function ClearReport() {
    ...
    $('.clsReportH').hide();
    $('.tbodybudgetsummarywithincreaseListing').html('');
