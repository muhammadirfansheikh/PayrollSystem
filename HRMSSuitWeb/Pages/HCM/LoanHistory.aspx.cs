...
public partial class Pages_HCM_LoanHistory : System.Web.UI.Page
{
    ...
    private void BindLoanMaster(DataTable dt) {
        ...
        if (dt.Rows.Count> 0)
        {
            ...
            lblLoanApplyDate.Text =Convert.ToDateTime( dt.Rows[0]["CreatedDate"]).ToString("yyyy-MM-dd hh:MM:ss");
            lblSanctionDate.Text = dt.Rows[0]["SanctionDate"] != DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["SanctionDate"]).ToString("yyyy-MM-dd hh:MM:ss") : ""; 
            lblSettlementDate.Text = dt.Rows[0]["SettlementDate"] != DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["SettlementDate"]).ToString("yyyy-MM-dd hh:MM:ss") : ""; 
            lblCurrentMonthInsTillDate.Text = dt.Rows[0]["CurrentMonthInstallmentTillDate"] != DBNull.Value ? Convert.ToDateTime(dt.Rows[0]["CurrentMonthInstallmentTillDate"]).ToString("yyyy-MM-dd hh:MM:ss") : "";
